    ...
    >
      <ul className="p-2">
        {filteredChildren.length > 0 ? filteredChildren : (
          <li className={clsx(
            "flex justify-center items-center p-2 text-[var(--color-fg)] opacity-50",
...
const ComboboxOption = ({ value, children }: ComboboxOptionProps) => {
  ...
  return (
    <li
      className={clsx(
        "flex justify-between items-center gap-2 cursor-pointer px-2 py-2.5 w-full rounded-md hover:bg-[var(--color-surface)] active:bg-[var(--color-surface)] font-body text-[var(--color-fg)] fill-current transition-colors",
        context?.size === "large" ? "text-base" : "text-sm"
      )}
